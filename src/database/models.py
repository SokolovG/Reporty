...
class DailyRecord(Base):
    ...
    bitrix_task_id: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="Bitrix task ID"
    )
...
class BitrixTask(Base):
    ...
    bitrix_id: Mapped[str] = mapped_column(
        String(50), unique=True, nullable=False, comment="Bitrix task ID"
    )
    ...
    # Date
    created_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="Creation date in Bitrix"
    )
    ...
    last_sync: Mapped[datetime] = mapped_column(
        DateTime, default=datetime.now(UTC), comment="Last synchronization"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<BitrixTask(bitrix_id={self.bitrix_id}, title='{self.title}',"
...
class Report(Base):
    ...
    generated_at: Mapped[datetime] = mapped_column(
        DateTime, default=datetime.now(UTC), comment="Generation time"
    )
